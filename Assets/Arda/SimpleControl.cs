...
public class SimpleControl : MonoBehaviour
{
    ...
    public float radius;
    //Distance between player and guard
    float dist;
    ...
    public void Update()
    {
        //If the distance is bigger than the radius than the navmesh will return to it's pathfinding
        if(dist > radius)
        {
            _navMeshAgent.isStopped = false;
        }

        if (GetComponent<View>().CanSeePlayer())
        {
    ...
    private void SetPlayerDestination()
    {
        ...
        Debug.Log(dist);
        //If distance is less than the radius, than the navmesh's pathfinding will come to a stop.
        if (dist < radius)
        {
